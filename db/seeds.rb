...
4.times do
  ...
  (1+rand(20)).times do
    ...
    (1+rand(20)).times do
      card = Card.create(name:Faker::Name.name,bio:Faker::Lorem.paragraph,setting:deck.setting,tag_line:Faker::Lorem.sentence,user_id:1+rand(3))
      deck.cards << card
    end

    (1+rand(30)).times do
      pass = Passage.create(user_id:1+rand(3), content:Faker::Lorem.paragraph)
      deck.passages << pass
    end
    user.decks << deck
  end
